...
import {
  ...
  resetSoccerBacktestShapeCacheForTests,
  soccerBacktestLiveGateBlock,
...
describe("soccer backtest live gate", () => {
  ...
  it("allows backtest-positive 3.5-4.5 match totals at cheap costs", () => {
    resetSoccerBacktestShapeCacheForTests();
    const shapes = loadSoccerBacktestPositiveShapes();
    expect(shapes.has("match_total|3.5-4.5|w1")).toBe(true);
    expect(soccerBacktestLiveGateBlock(soccerCandidate(), "match_total")).toBeNull();
  });
  ...
  it("blocks 4.5-5.5 match totals with negative backtest ROI@worst", () => {
    ...
    const block = soccerBacktestLiveGateBlock(
      soccerCandidate({ broad, narrow, packageCost: 1.005 }),
      "match_total",
    );
    expect(block).toMatch(/soccer_backtest_shape_not_positive/);
  });

  it("blocks costs above the backtest positive-ROI ceiling for a shape", () => {
    resetSoccerBacktestShapeCacheForTests();
    const block = soccerBacktestLiveGateBlock(soccerCandidate({ packageCost: 1.25 }), "match_total");
    expect(block).toMatch(/soccer_backtest_cost_above_positive_roi/);
  });
});
